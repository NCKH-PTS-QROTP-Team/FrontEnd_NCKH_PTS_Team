...
import { importService, ImportStudentResponse, ImportError } from "@/apis";
import Toast, { useToast } from "@/components/Toast";
import { UploadIcon, CheckCircleIcon, UsersIcon } from "@/components/Icons";

export default function ImportStudentsScreen() {
  const [file, setFile] = useState<File | null>(null);
  const [loading, setLoading] = useState(false);
  const [importResult, setImportResult] = useState<ImportStudentResponse | null>(null);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (result.errorCount === 0) {
        showToast(
          `Import thành công ${result.successCount} sinh viên!`,
          "success"
        );
      } else {
        showToast(
      ...
      >
        ...
        >
          ...
          <View style={{ marginBottom: 24 }}>
            ...
            >
              Tải lên file Excel chứa thông tin các sinh viên để import hàng loạt
            </Text>
          </View>
          ...
          >
            ...
            >
              Lưu ý: Dòng đầu tiên là header, các dòng tiếp theo là dữ liệu. Mật khẩu mặc định: student123
            </Text>
          </View>
