...
export default function AttendanceMonitoringScreen() {
    ...
    const filteredSessions = useMemo(() => {
        return sessions.filter(session => {
            // Filter by date
            const sessionDate = session.startTime ? session.startTime.substring(0, 10) : '';
            if (sessionDate !== selectedDate) return false;

            // Filter by search query
            if (searchQuery) {
                const q = searchQuery.toLowerCase();
                const matchClass = session.className?.toLowerCase().includes(q) || false;
                const matchSubject = session.subjectName?.toLowerCase().includes(q) || false;
            ...
            >
                <View style={styles.cardHeader}>
                    ...
                    <Text style={styles.timeText}>
                        <Ionicons name="time-outline" size={14} color="#64748b" /> {formatTime(item.startTime)}
                    </Text>
                </View>

                <View style={styles.cardBody}>
                    <Text style={styles.subjectName}>{item.subjectName || 'Không có tên môn học'}</Text>
                    <Text style={styles.className}>{item.className || 'Không có tên lớp'}</Text>

                    <View style={styles.metaRow}>
                        <Ionicons name="location-outline" size={14} color="#94a3b8" />
                        <Text style={styles.metaText}>{item.description || 'Chưa xếp phòng'}</Text>
                    </View>
                </View>
                    ...
                    <View style={styles.cardFooter}>
                        <View style={styles.progressWrap}>
                            <View style={styles.progressHeader}>
                                ...
                                <Text style={styles.progressValue}>{present}/{total} ({Math.round(present / total * 100)}%)</Text>
                            </View>
                            <View style={styles.progressBar}>
                                <View style={[styles.progressFill, { width: `${(present / total) * 100}%` }]} />
                            </View>
                        </View>
                    </View>
                )}
        ...
        <View style={styles.container}>
            ...
            {loading ? (
                ...
                <View style={styles.centerBox}>
                    ...
                    <Text style={styles.emptyText}>Không có phiên điểm danh nào trong ngày này</Text>
                    {selectedDate !== todayISO && (
                        <TouchableOpacity style={styles.todayBtn} onPress={() => setSelectedDate(todayISO)}>
                        ...
                        <View style={styles.modalContent}>
                            ...
                            <ScrollView style={styles.modalBody}>
                                <View style={styles.detailRow}>
                                    ...
                                    <Text style={styles.detailValue}>{selectedSession.subjectName || 'N/A'}</Text>
                                </View>
                                <View style={styles.detailRow}>
                                    <Text style={styles.detailLabel}>Lớp học:</Text>
                                    <Text style={styles.detailValue}>{selectedSession.className || 'N/A'}</Text>
                                </View>
                                <View style={styles.detailRow}>
                                    <Text style={styles.detailLabel}>Phương thức:</Text>
                                    <Text style={styles.detailValue}>{selectedSession.method || 'Mặc định'}</Text>
                                </View>
                                <View style={styles.detailRow}>
                                    <Text style={styles.detailLabel}>Bắt đầu:</Text>
                                    <Text style={styles.detailValue}>{formatTime(selectedSession.startTime)}</Text>
                                </View>
                                {selectedSession.endTime && (
                                    <View style={styles.detailRow}>
                                        <Text style={styles.detailLabel}>Kết thúc:</Text>
                                        <Text style={styles.detailValue}>{formatTime(selectedSession.endTime)}</Text>
                                    </View>
                                )}
                                ...
                                <View style={styles.statsGrid}>
                                    <View style={[styles.statBox, { backgroundColor: '#ecfdf5', borderColor: '#d1fae5' }]}>
                                        <Text style={[styles.statBoxTitle, { color: '#059669' }]}>Có mặt</Text>
                                        <Text style={[styles.statBoxValue, { color: '#059669' }]}>{selectedSession.present ?? 0}</Text>
                                    </View>
                                    <View style={[styles.statBox, { backgroundColor: '#fff7ed', borderColor: '#ffedd5' }]}>
                                        <Text style={[styles.statBoxTitle, { color: '#ea580c' }]}>Đi trễ</Text>
                                        <Text style={[styles.statBoxValue, { color: '#ea580c' }]}>{selectedSession.late ?? 0}</Text>
                                    </View>
                                    <View style={[styles.statBox, { backgroundColor: '#fef2f2', borderColor: '#fee2e2' }]}>
                                        <Text style={[styles.statBoxTitle, { color: '#dc2626' }]}>Vắng mặt</Text>
                                        <Text style={[styles.statBoxValue, { color: '#dc2626' }]}>{selectedSession.absent ?? 0}</Text>
                                    </View>
                                </View>
                                ...
                                <View style={styles.infoBox}>
                                    ...
                                    <Text style={styles.infoText}>Tính năng xem chi tiết điểm danh của từng sinh viên sẽ được cập nhật trong phiên bản tiếp theo.</Text>
                                </View>
                            </ScrollView>
